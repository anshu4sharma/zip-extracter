...
import { ChangeEvent, DragEvent, Fragment, Suspense, useCallback, useState } from 'react';
      ...
      <div className='flex flex-col items-center'>
        <div className="flex items-center justify-center w-80 sm:w-full max-w-md flex-col relative">
          <label onDrop={handleDrop} onDragEnter={handleDragOver} onDragEnd={handleDragOver}
            ...
            onDragOver={handleDragOver} htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-bray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
            {selectedFile ? (
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-8 h-8 mb-2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                </svg>
                <p className="mb-2 text-center text-sm text-gray-500 dark:text-gray-400">{selectedFile.name}</p>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center pt-5 pb-6">
        ...
        <div className='grid grid-cols-2 place-content-center'>
          <button type="button" onClick={() => saveAsZip(selectedFile)} disabled={!selectedFile} className="text-white w-100 mt-4 max-w-md bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 disabled:opacity-50">UnZip File </button>
          {selectedFile && <button type="button" onClick={displayZipFile} disabled={!selectedFile} className="text-white w-100 mt-4 max-w-md bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800 disabled:opacity-50"> Show Files </button>}
        </div>
